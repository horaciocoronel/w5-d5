...
# puts hash[:response][:results].inspect


def read_article(hash)
  number = rand(0..9)
  add_views = hash[:response][:results][number][:views]
  add_views += 1
  hash[:response][:results][number][:views] = add_views
  # puts hash[:response][:results]
end

def display_views(hash)
  hash[:response][:results].each do |article|
    puts article[:webTitle] + ' Count: ' + article[:views].to_s
  end
end
